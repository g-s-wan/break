...
/**
 * This tests that the bound method works when given clearly valid bounds. 
 */
test("filter simple", async () => {
  let user = userEvent.setup();
  await userEvent.type(input, "bound 180 180 -180 -180");
...
test("filter no parameters error", async () => {
  let user = userEvent.setup();
  await userEvent.type(input, "bound");
  await user.click(button);
...
test("filter non-numerical parameters given", async () => {
  let user = userEvent.setup();
  await userEvent.type(input, "bound a b c d");
  await user.click(button);
  expect(history.textContent).toBe(
    "Error: latitude and longitude values should be numbers!"
  );
});
...
test("search simple", async () => {
  let user = userEvent.setup();
  await userEvent.type(input, "searchjson hi");
  await user.click(button);
  expect(history.textContent).toBe("Command Success");
});
...
test("search no parameters", async () => {
  let user = userEvent.setup();
  await userEvent.type(input, "searchjson");
  await user.click(button);
  expect(history.textContent).toBe(
...
test("search too many parameters", async () => {
  let user = userEvent.setup();
  await userEvent.type(input, "searchjson hi yuh");
  await user.click(button);
  expect(history.textContent).toBe(
    "Error: Searchjson command needs 1 argument: keyword(s) Hint: Use %20 where you would have spaces."
  );
...
test("entering with button", async () =>{
  let user = userEvent.setup();
  await userEvent.type(input, "bound 1 2 3 4");
